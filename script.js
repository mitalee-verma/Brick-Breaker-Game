...
let blockWidth = 50;
let blockHeight = 10;
let blockColumns; // Will be calculated based on screen size
let blockRows; // Will be calculated based on screen size
...
function resizeCanvas() {
    ...
    // Recalculate block columns and rows
    blockColumns = Math.floor((boardWidth - blockX * 2) / (blockWidth + 10));
    blockRows = Math.min(Math.floor((boardHeight - blockY * 2) / (blockHeight + 10)), blockMaxRows);
...
function update() {
    ...
    if (ball.y <= 0) {
        ...
        context.fillText("Game Over: Press 'Space' to Restart", boardWidth / 2 - 140, boardHeight / 2);
        gameOver = true;
    }
...
function resetGame() {
    ...
    blockArray = [];
    score = 0;
    resizeCanvas(); // Adjust canvas size and recreate blocks
}
